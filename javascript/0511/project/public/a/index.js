...
oUserName.oninput = function () {
    ...
    if (/^\w{8,16}$/.test(this.value.trim())) {
...
oSubmit.onclick = function (e) {
    if (!oTip_userName.style.color || !oTip_passWord.style.color) e.preventDefault();
    if (oTip_userName.style.color === "red" || oTip_passWord.style.color === "red") e.preventDefault();
}